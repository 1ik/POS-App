...
var t = new Transfer();
var itemsIDListed = new Array(); //contains the list of ids that have been added.
...
$(document).ready(function(){
    ...
    $('#purchase_id_form').on('submit' , function(){
        ...
        $.get("../items/get_items_by_purchase_id/"+purchase_id , function(json_response){
            ...
            for (var i=0; i<items.length; i++){
                ...
                var item = items[i];
                if(itemsIDListed[item.id] === "1") {
                    listed_items += item.id + ",";
                    continue;
                }
                t.addItem(item.id,item.sell_price);

                itemsIDListed[item.id] = "1";
            ...
            if(listed_items != "")
                alert(listed_items + " Items not added because they are already listed.");
    ...
    $('#barcode_form').on('submit' , function() {
        ...
        $.get('../items/get_items_json/'+barcode , function(json_response){
            ...
            if($item == null){
                ...
                if(itemsIDListed[$item.id] === "1") { //checking if the list contains the item.
                    alert("Item is already listed in the table");

                } else {
                    itemsIDListed[$item.id] = "1"; //we add the item in the list.
    ...
    $("table").on("click", "span.glyphicon", function(){
        itemsIDListed[t.items[$(this).attr('id')]] = 1; //get the item id from index and then set 
