...
class Expense extends CI_Controller{
    ...
    public function index() {
        if($this->input->get()) {
            $data['vars']['expenses'] = $this->expense_model->get_expenses();
        }
    ...
    public function edit($expense_id) {
        ...
        if($this->input->post()) {
            $this->expense_model->update($expense_id);
        }

        $expense = $this->expense_model->get("*",array('id' => $expense_id));
        $data['vars']['expense'] = $expense[0];
        $data['vars']['expense_types'] = $this->expense_type_model->get();
        $data['vars']['showrooms'] = $this->showroom_model->get();

        $data['page_name'] = 'Edit Expense';
        $data['main_content'] = 'expense/edit';
        $this->load_view($data);
    }
    ...
    public function delete($expense_id) {
        $this->expense_model->delete($expense_id);
        $this->index();
    }
