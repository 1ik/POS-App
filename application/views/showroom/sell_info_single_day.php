        ...
        <div class="table table-responsive">
            <table class="table-bordered table table-bordered">
                ...
                <?php 
                    ...
                    $total_returned=0; 
                    $sold_price_total=0; 
                    $return_price_total=0;  
                    $discount_total = 0;
                    $total_generated_bill = 0; 
                ?>

                <?php foreach ($memos as $memo) { ?>
                    <?php $sell = isset( $memo["sell"] ) ? $memo["sell"] : ''; ?>
                    <?php $return = isset( $memo["return"] ) ? $memo["return"] : ''; ?>

                    <tr>
                        <td> <?php if( $sell != '' ) echo $sell->id;?> </td>
                        <td> <?php if( $sell != '' ) echo $sell->memo_token; ?></td>
                        <td> <?php if( $sell != '' ) echo $sell->time; ?> </td>
                        <td> <?php if( $sell != '' ) echo $sell->salesman; ?> </td>
                        <td> <?php if( $sell != '' ) echo $sell->item_count; ?> </td>
                        <td> <?php if( $sell != '' ) echo $sell->cash_billed; ?> </td>
                        <td> <?php if( $return != '' ) echo $return->return_count; ?> </td>
                        <td> <?php if( $return != '' ) echo $return->return_amount; ?> </td>

                        <?php 
                            $cash_billed =   $sell != '' ? $sell->cash_billed : 0; 
                            $return_amount = $return != '' ? $return->return_amount : 0;
                        ?>
                        <td> <?php echo $cash_billed - $return_amount; ?> </td>

                        <?php $discount =  $sell != '' ? $sell->discount : 0; ?>
                        <td> <?php echo $discount; ?> </td>
                        <?php $id = $sell != '' ? $sell->id : $return->id ;?>
                        <th><?php echo anchor('showroom/memo/'.$id."/detail" , 'detail' , array('target' => '_blank')); ?></th>
                        <?php

                            $total_sold += $sell != '' ? $sell->item_count : 0;
                            $total_returned += $return != '' ? $return->return_count : 0;
                            $sold_price_total += $sell != '' ? $sell->cash_billed : 0;
                            $return_price_total += $return != '' ? $return->return_amount : 0;
                            $discount_total += $sell != '' ? $sell->discount : 0;
                            $total_generated_bill += ($cash_billed - $return_amount);
                        ?>                    
                    <tr/>
